...
export class AppLayoutComponent implements OnDestroy {
  ...
  regex_strong: string = '^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)(?=.*[@$!%*?&])[A-Za-z\\d@$!%*?&]{8,}';
	...
	ngOnInit(){
      ...
      next: (response : any) => {
				if(response.invalid){
					this.authService.sendMessage('info', 'Alterar Senha', 'A sua senha no momento é a senha padrão do sistema, é necessário alterá-la.');
  ...
  onSavePassword(){
    ...
    this.authService.updatePassword('', this.new_password, true);
